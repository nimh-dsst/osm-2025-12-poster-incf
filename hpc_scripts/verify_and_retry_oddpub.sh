#!/bin/bash
# Verify oddpub output completeness and generate retry swarm for failed jobs
# Usage: bash verify_and_retry_oddpub.sh <tar_dir> <output_dir> <container_sif>
...
RETRY_SWARM_FILE="oddpub_retry_swarm.txt"
> "$RETRY_SWARM_FILE"
    ...
    if [ "$xml_count" -gt "$CHUNK_SIZE" ]; then
        ...
        for ((chunk=0; chunk<num_chunks; chunk++)); do
            ...
            if [ ! -f "$output_file" ]; then
                ...
                echo ". /usr/local/current/apptainer/app_conf/sing_binds && apptainer exec $CONTAINER_SIF python3 /scripts/process_pmcoa_with_oddpub.py --batch-size 500 --start-index $start_index --chunk-size $CHUNK_SIZE --output-file $output_file $tarfile" >> "$RETRY_SWARM_FILE"
            fi
        done
        ...
        if [ ! -f "$output_file" ]; then
            ...
            echo ". /usr/local/current/apptainer/app_conf/sing_binds && apptainer exec $CONTAINER_SIF python3 /scripts/process_pmcoa_with_oddpub.py --batch-size 500 --output-file $output_file $tarfile" >> "$RETRY_SWARM_FILE"
        fi
    fi
done
